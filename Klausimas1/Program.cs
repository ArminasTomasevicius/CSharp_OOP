...
namespace Klausimas1
{
    class Krepsininkas
    {
        ...
        {
            int v1 = string.Compare(k1.ImtiVarda, k2.ImtiVarda, StringComparison.CurrentCulture);

                return (k1.ImtiTaskus >= k2.ImtiTaskus || (k1.ImtiTaskus == k2.ImtiTaskus && v1>0));
        }
    ...
    class Komanda
    {
        ...
        public void Iterpti(Krepsininkas krepsininkas)
        {
            for (int i = 0; i < ImtiN; i++)
            {
                if (komanda[i] >= komanda[i+1])
                {
                    komanda[i] 
                }
            }
        ...
        public void Vid(Krepsininkas krepsininkas)
        {
            sum = 0;
            kiek = 0;
            for (int i = 0; i < ImtiN; i++)
            {
                if (krepsininkas.ImtiUgi > y)
                {
                    sum += krepsininkas.ImtiTaskus;
                    kiek++;
                }
            }
           double vid = sum / kiek;
